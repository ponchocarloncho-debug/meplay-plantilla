...
"""
================================================================================
VIDEO PAGES GENERATOR - SIMPLE & CLEAN VERSION
================================================================================
Este generador crea páginas de video simples que SÍ funcionan.
================================================================================
"""
...
CONFIG = {
    'SIDEBAR_RANDOM': 10,     # Cantidad de videos Random en sidebar (recomendado: 6)
    'SIDEBAR_RECENTS': 4,    # Recents iniciales (recomendado: 4, se cargan más con "Load More")
}
# ====================================================

def load_data():
    with open('videodata.json', 'r', encoding='utf-8') as f:
        return json.load(f)

def create_page(video):
    ...
    embed_url = video.get('embed', '')
    
    # Generate ID same way as widget.js: "videos/diego-yoga.html" -> "diego-yoga"
    video_id = url.split('/')[-1].replace('.html', '')
    
    # Encode embed
    encoded_embed = base64.b64encode(embed_url.encode()).decode()
    
    # Tags HTML
    tags_html = ""
    for tag in tags:
        tags_html += f'        <span class="tag" onclick="searchByTag(\'{tag}\')">{tag}</span>\n'
    
    # Gallery
    gallery_html = ""
    for img in images[:6]:
        gallery_html += f'        <img src="{img}" class="gallery-img" loading="lazy">\n'
    
    # Hidden gallery
    hidden_html = ""
    if len(images) > 6:
        hidden_html = '      <div class="hidden-gallery" id="hiddenGallery" style="display:none;">\n'
        for img in images[6:]:
            hidden_html += f'        <img src="{img}" class="gallery-img" loading="lazy">\n'
        hidden_html += '      </div>\n'
        hidden_html += f'      <button class="show-more-btn" onclick="toggleGallery()">Show {len(images)-6} More Images</button>\n'
...
<body>
  ...
  <img src="{thumbnail}" alt="{title}" style="display:none;" id="primary-thumbnail">
  ...
  <div class="main-container">
    <div class="video-section">
      ...
      <div class="video-player">
        <div class="video-thumbnail-overlay" id="thumbnail-overlay" style="background-image: url('{thumbnail}');">
...
{tags_html}      </div>
  ...
  <script>
    const videoId = '{video_id}';  // Use same format as widget.js: "diego-yoga"
    const videoTitle = '{title}';
    const encodedEmbed = '{encoded_embed}';
    ...
    const SIDEBAR_RECENTS_PER_LOAD = {CONFIG['SIDEBAR_RECENTS']};
    ...
    function loadSidebar() {{
      ...
      const random = window.getRandomVideos({CONFIG['SIDEBAR_RANDOM']}, videoId);
      
      let html = '<h3>🎲 Random</h3><div class="sidebar-grid">';
      random.forEach(v => {{
    ...
    function loadSidebarRecents() {{
      ...
      let html = '<h3>🕒 Recents</h3><div class="sidebar-grid" id="recentsGrid">';
      ...
      container.innerHTML = container.innerHTML.split('<h3>🕒 Recents</h3>')[0] + html;
    }}
    ...
    window.showSearchResults = function(query) {{
      ...
      let html = '<div style="padding: 20px; background: #1a1a1a; border-bottom: 2px solid #e50914;"><strong style="color: #e50914; font-size: 20px;">🔍 Found ' + matches.length + ' results</strong></div>';
...
def main():
    print("🎬 Generador LIMPIO de Páginas")
    print("=" * 50)
    
    videos = load_data()
    print(f"✓ {len(videos)} videos cargados")
    ...
    print(f"\n✅ {len(videos)} páginas generadas")
    print("\n🚀 Ejecuta: python3 generate_pages_CLEAN.py")
